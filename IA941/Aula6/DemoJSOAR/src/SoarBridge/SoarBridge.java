...
public class SoarBridge {
    ...
    Environment env;
    public Creature c;
    public List<Thing> foodMemory = new ArrayList<Thing>();
    public List<Thing> jewelMemory = new ArrayList<Thing>();
    ...
    private void addToMemory(Thing thing) {
        ...
        if( type.equalsIgnoreCase("FOOD")){
            if (!foodMemory.contains(thing)) foodMemory.add(thing);
        }
        else if (type.equalsIgnoreCase("JEWEL")){
            if (!jewelMemory.contains(thing)) jewelMemory.add(thing);
        }
    ...
    private Leaflet getMostValuableLeaflet(List<Leaflet> leaflets) {
        ...
        for (Leaflet l : leaflets) {
            if (l.getPayment() > price && !l.isCompleted()) {
                leaflet = l;
                price = l.getPayment();
            }
        }
            ...
            for (Command command : commandList) {
                switch (command.getCommandType()) {
                    ...
                    case GET:
                        ...
                        jewelName = ((CommandGet) command.getCommandArgument()).getThingName();
                        for(Thing j: jewelMemory){
                            if(jewelName.equals(j.getName())){
                                foodMemory.remove(j);
                                break;
                            }
                        }
                    ...
                    case EAT:
                        ...
                        foodName = ((CommandEat) command.getCommandArgument()).getThingName();
                        for(Thing f: foodMemory){
                            if(foodName.equals(f.getName())){
                                foodMemory.remove(f);
                                break;
                            }
                        }
                    ...
                    case HIDE:
                        ...
                        jewelName = ((CommandHide) command.getCommandArgument()).getThingName();
                        for(Thing j: jewelMemory){
                            if(jewelName.equals(j.getName())){
                                foodMemory.remove(j);
                                break;
